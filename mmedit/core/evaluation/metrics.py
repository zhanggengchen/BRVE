...
from skvideo.measure import strred as compare_strred
...
def psnr(img1, img2, format='raw'):
    if format == 'raw':
        return compare_psnr(image_test=img1, image_true=img2, data_range=1.0)
    
    if format == 'rgb':
        return compare_psnr(image_test=img1, image_true=img2, data_range=255)

def ssim(img1, img2, format='raw'):
    if format == 'raw':
        test_raw_ssim = 0
        for i in range(4):
            test_raw_ssim += compare_ssim(img1[:, :, i], img2[:, :, i], data_range=1.0)
        return test_raw_ssim/4
    
    if format == 'rgb':
        test_rgb_ssim = 0
        for i in range(3):
            test_rgb_ssim += compare_ssim(img1[:, :, i], img2[:, :, i], data_range=255)
        return test_rgb_ssim/3

    # return compare_ssim(img1, img2, channel_axis=channel_axis)

def strred(vid1, vid2, format='raw'):
    # print(vid1.shape, vid1.dtype)
    if format == 'raw':
        test_raw_strred = 0
        for i in range(4):
            test_raw_strred += compare_strred(vid1[:, :, :, i], vid2[:, :, :, i])[1]
        return test_raw_strred / 4.0
    
    if format == 'rgb':
        test_rgb_strred = 0
        for i in range(3):
            test_rgb_strred += compare_strred(vid1[:, :, :, i] / 255.0, vid2[:, :, :, i] / 255.0)[1]
        return test_rgb_strred / 3.0
    
def mse_mabd(vid1, vid2, format='raw'):
